...
class DownloadFile
{
    ...
    private static function createTempFile($prefix = 'file')
    {
retryTmp:    
        $tmpPath = tempnam(self::$TMP_ROOT, $prefix);
    ...
    public static function loadAll($url, $all, $prefix = 'file')
    {
        $tasks = [];
        array_walk($all, function(&$item, $index) use (&$tasks, $prefix) {
            $cachePath = APP_PATH . "/runtime/cache/" . $prefix.'/'.$item;
        ...
        if (count($tasks) > 0) {
            ...
            $contexts = [];
            $retried = false;
            array_walk($tasks, function(&$index, $task) use ($mh, &$contexts, &$retried, $url, $prefix) {
                $handle = curl_init();
                $tmpPath = self::createTempFile($prefix);
                $file = $tmpPath[1];
                $tmpPath = $tmpPath[0];
            ...
            array_walk($contexts, function(&$context, $task) use($mh, $tasks, &$all, $prefix) {
                $task = $context[1];
                $tmpPath = $context[2];
                $fileSize = filesize($tmpPath);
//                 var_dump($fileSize);
//                 var_dump( Us\Config\CONTENT_GENERATOR_FILE_SIZE_LIMIT);
...
//                 }
                ...
                $path = APP_PATH . "/runtime/cache/" . $prefix . '/'. $task;
